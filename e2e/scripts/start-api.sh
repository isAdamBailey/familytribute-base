...
# Boots the seeded Laravel JSON API on :8000 for Playwright. Paired with
# start-nuxt.sh via playwright.config.ts's array `webServer` config — Nuxt
# (the only frontend since the issue #19 cutover) is the thing being tested,
# this is its backend. Fixed ports (matching .env.e2e.example's APP_URL/
# FRONTEND_URLS/SANCTUM_STATEFUL_DOMAINS) rather than configurable ones —
